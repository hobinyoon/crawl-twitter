...
namespace Ops {
	...
	void GenTopTopics() {
		Util::CpuTimer _("Generate top topics ...\n");
		...
		map<int, vector<string> > cnt_topic;
		for (auto tc: topic_cnt) {
			int cnt = tc.second;
			const string& t = tc.first;

			auto it = cnt_topic.find(cnt);
			if (it == cnt_topic.end()) {
				vector<string> v;
				v.push_back(t);
				cnt_topic[cnt] = v;
		...
		const string& fn = Conf::fn_top_topics;
		{
			ofstream ofs(fn);
			if (! ofs.is_open())
				throw runtime_error(str(boost::format("unable to open file %1%") % fn));
			...
			int i = 0;
			for (auto it = cnt_topic.rbegin(); it != cnt_topic.rend(); it ++) {
				int cnt = it->first;
