...
namespace Conf {
	...
	void _ParseArgs(int argc, char* argv[]) {
		...
		od0.add_options()
			...
			("load_from", po::value<string>()->default_value("file"), "Load from: file or db")
		...
		{
			load_from = vm["load_from"].as<string>();
			if (load_from != "file" && load_from != "db")
				throw runtime_error(str(boost::format("Invalid load_from: %s") % load_from));
		}
